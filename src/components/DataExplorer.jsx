    ...
    <div className="flex flex-col h-full bg-black/20 backdrop-blur-3xl p-6">
      ...
      <div className="space-y-6 mb-8">
         <div className="flex items-center justify-between">
            <div>
               <h3 className="text-lg font-black text-white italic tracking-tighter uppercase underline decoration-indigo-500/50 underline-offset-8 decoration-4">Source Discovery</h3>
            </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pb-8">
        {!hasMetadata ? (
          <div className="h-60 flex flex-col items-center justify-center py-10 px-8 text-center glass-card rounded-[40px] border-dashed border-white/5">
             ...
             <h4 className="text-xs font-black text-slate-300 uppercase tracking-widest mb-1 italic">No Source Ingested</h4>
             <p className="text-[10px] text-slate-600 font-medium leading-relaxed max-w-[180px]">Run the pipeline explorer to analyze source metadata.</p>
          </div>
        ) : (
          <div className="space-y-4">
             <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-4 flex flex-col gap-1">
                   <Activity size={14} className="text-emerald-400" />
                   <span className="text-lg font-black text-white italic tracking-tighter">{tableNames.length} TABLES</span>
                   <span className="text-[9px] font-black text-emerald-400/60 uppercase tracking-widest">Detected</span>
                </div>
                <div className="bg-white/[0.03] border border-white/[0.08] rounded-3xl p-4 flex flex-col gap-1">
                   <Target size={14} className="text-slate-500" />
                   <span className="text-lg font-black text-white italic tracking-tighter">RAW</span>
                   <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Metadata</span>
                </div>
             </div>
      ...
      <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2">
             <div className={`w-2 h-2 rounded-full ${hasMetadata ? 'bg-emerald-500' : 'bg-slate-800'}`} />
             <span className="text-[9px] font-black text-slate-700 uppercase tracking-[0.2em]">SOURCE HUD</span>
          </div>
      </div>
    </div>
  );
